 ...
 */
 int bytes_vit_cnt = 0, bytes_pher_cnt = 0, bytes_robot_cnt = 0, bytes_stream_cnt = 0;
...
void task_communicate(void)
{
	...
	if(g_task_communicate.enabled == s_TRUE)
	{
		...
		//	LAB 2 starts here
		// --------------------------------------------------
		int bytes_sent_cnt = 0;


		doublylinkedlist_t *pheromone_data = doublylinkedlist_init();
		doublylinkedlist_t *stream_data = doublylinkedlist_init();

		doublylinkedlist_t **supplementing_list;
		...
		while(g_list_send->count != 0)
		{
			...
			case s_DATA_STRUCT_TYPE_PHEROMONE :
				...
				supplementing_list = &pheromone_data;
				break;
			...
			case s_DATA_STRUCT_TYPE_STREAM :
				...
				supplementing_list = &stream_data;
				break;
			// Other
			default :
				// Do nothing
	...
	doublylinkedlist_insert_beginning(*supplementing_list, data, data_type);
	...
	doublylinkedlist_insert_beginning(*supplementing_list, data, data_type);
            ...
            bytes_vit_cnt = udp_packet_len;
      			bytes_sent_cnt += udp_packet_len;
            clock_gettime(CLOCK_REALTIME, &time2);
            vit_time = 1e+3*(time2.tv_sec - time1.tv_sec) + 1e-6 * (time2.tv_nsec - time1.tv_nsec);
            printf("finding_victim_time :%f\n", vit_time);
					}
					else if (data_type == s_DATA_STRUCT_TYPE_ROBOT){

            // Broadcast packet
      			udp_broadcast(g_udps, udp_packet, udp_packet_len);
            bytes_robot_cnt = udp_packet_len;
      			bytes_sent_cnt += udp_packet_len;
					}
		...
		while (pheromone_data->count != 0 && bytes_sent_cnt < 2350) {
			data = (void *)malloc(sizeof(pheromone_map_sector_t));
			doublylinkedlist_remove(pheromone_data, pheromone_data->first, data, &data_type);
		...
		// Broadcast packet
		udp_broadcast(g_udps, udp_packet, udp_packet_len);
		bytes_pher_cnt = udp_packet_len;
		free(data);
			}
			//free(data);
		}

		doublylinkedlist_destroy(pheromone_data);

		while(stream_data->count != 0 && bytes_sent_cnt < 2350) {

			data = (void *)malloc(sizeof(stream_t));
			doublylinkedlist_remove(stream_data, stream_data->first, data, &data_type);
		...
		// Broadcast packet
		udp_broadcast(g_udps, udp_packet, udp_packet_len);
		bytes_stream_cnt = udp_packet_len;
		free(data);
			}
			//free(data);
		}

		doublylinkedlist_destroy(stream_data);
